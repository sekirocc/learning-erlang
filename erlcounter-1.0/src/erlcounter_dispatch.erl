...
-define (POOL, elcounter).
-record (data, {regex=[], refs=[] }).

start_link() ->
  gen_fsm:start_link(?MODULE, [], []).

complete(Pid, Reges, Ref, Count) ->
  gen_fsm:send_all_state_event(Pid, {complete, Reges, Ref, Count}).

init([]) ->
  {ok, Re} = application:get_env(regex),
  {ok, Dir} = application:get_env(directory),
  {ok, MaxFiles} = application:get_env(max_files),
  ppool:start_pool(?POOL, MaxFiles, {erlcounter_counter, start_link, []}),
...
handle_info({start, Dir}, State, Data) ->
  gen_fsm:send_event(self(), erlcounter_lib:find_erl(Dir)),
...
listening(done, #data{regex=Re, refs=[]}) ->
  [io:format("Regex ~s have ~p results~n", [R, C]) || {R, C} <- Re],
  {stop, normal, done};
...
handle_sync_event(Event, _From, State, Data) ->
  io:format("unkown event ~p~n", [Event]),
  {next_state, State, Data}.
