...
handle_info(Msg, S) ->
  io:format("unkown message ~s~n", [Msg]),
  {noreply, S}.

handle_worker_down(Ref, S=#state{limit=N, sup=Sup, refs=Refs}) ->
  case queue:out(S#state.queue) of
    ...
    {{value, {Args}}, Q} ->
      ...
      {noreply, S#state{refs=NewRefs, queue=Q}};
    {empty, _} ->
      {noreply, S#state{limit=N+1, refs=gb_sets:delete(Ref, Refs)}};
    Msg ->
      io:format("not match!!!!!!!!~p~n", [Msg]),
      {noreply, S#state{limit=N+1, refs=gb_sets:delete(Ref, Refs)}}
  end.
